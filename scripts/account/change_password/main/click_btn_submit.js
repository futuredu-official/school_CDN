async function click_btn_submit() {
	if (!toggle_click_btn_submit) {
		...
		if (data.exist == "EXIST") {
			...
			ReactDOM.render(<Div_desc_err_msg text="이메일의 '받은 편지함'을 확인해주세요."/>, document.getElementById("desc_email_msg"))
			...
			document.getElementById("desc_email_msg").className = class_desc_msg
			ReactDOM.render(<Div_desc_err_msg text="해당 계정은 존재하지 않습니다."/>, document.getElementById("desc_email_msg"))
